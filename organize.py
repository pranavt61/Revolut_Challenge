...
def order_table_token(wallet, tokens, web3):
    ...
    for t in tokens:
        ...
        token_entry[t["symbol"]] = str(balance)
    return token_entry

def execute_sql(table_quick, table_tx, table_block, table_tokens, token_list):
    import os
    ...
    if db_is_new:
        ...
        create_database(cur, token_list)
        create_index(cur)
        update_database(cur,table_quick, table_tx, table_block, table_tokens)
    else:
        update_database(cur,table_quick, table_tx, table_block, table_tokens)
    conn.commit()
    conn.close()
